...
divElement.style.fontSize = '44px';
divElement.innerText = 'bruh'
...
for (let element of coursesAndDurationArray1){
    ...
    divs.innerText = element.title + ' ' + element.monthDuration
...
for (let elem of coursesArray){
    ...
    let p = document.createElement('p')
    p.innerText = 'monthDuration: ' + elem.monthDuration + ' hourDuration: ' + elem.hourDuration

    let ul = document.createElement('ul')

    divvv.append(h1, p, ul)

    for (let j = 0; j < elem.modules.length; j++){
        let li = document.createElement('li')
        li.innerText = elem.modules[j]
        ul.appendChild(li)
    }
}